...
public class BufferReaderWriterUtilTest {
	...
	public void writeReadFileChannel() throws Exception {
		...
		validateTestBuffer(readToSegment(fc, readBuffer));
	}
	...
	private static Buffer readToSegment(FileChannel fc, MemorySegment segment) throws IOException {
		...
		assertNotNull(data);
		return data.build(Buffer.DataType.DATA_BUFFER, 1, 0).getBuffer(segment);
	}
}

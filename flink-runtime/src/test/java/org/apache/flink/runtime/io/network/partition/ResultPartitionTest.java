...
public class ResultPartitionTest {
	...
	public void testIdleTime() throws IOException, InterruptedException {
		...
		Buffer buffer = getBuffer(readView);
		...
		assertNotNull(getBuffer(readView));
	}
	...
	public void testFlushBoundedBlockingResultPartition() throws IOException {
		...
		for (int i = 0; i < 4; ++i) {
			assertEquals(record, getBuffer(readView1).getNioBufferReadable());
		}
		assertFalse(getBuffer(readView1).isBuffer());
		...
		for (int i = 0; i < 2; ++i) {
			assertEquals(record, getBuffer(readView2).getNioBufferReadable());
		}
		assertFalse(getBuffer(readView2).isBuffer());

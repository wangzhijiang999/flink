...
public class BoundedBlockingSubpartitionAvailabilityTest {
	...
	private static void drainAllData(ResultSubpartitionView reader) throws Exception {
		ResultSubpartitionView.PartitionData partitionData;
		while ((partitionData = reader.getNextData()) != null) {
			Buffer buffer = partitionData.getBuffer(MemorySegmentFactory.allocateUnpooledSegment(BUFFER_SIZE));
